...
, padding=1),
    ...
    # ------------------- LSTM Part -----------------
        ...
        self.moduleLong = torch.nn.Sequential(Basic(4, 32), torch.nn.AvgPool2d(kernel_size=2, stride=2))
    ...
    def forward(self, tensorInput1, tensorInput2, tensorLong=None, tensorResidual=None, ref_img=None, do_MA=False, only_MA=False):
        '''
        ...
        --------- [!] update 5.17 if only MA, only self.MA is called and all previous operation is skipped
        tensorInput1: output
        tensorInput2: warped
        '''
    ...
    # ------------------- LSTM Part --------------------
        ...
        '''
        below is the special case for only_MA
        '''
        if only_MA:
            tensorPred = tensorInput1
            ref_img = tensorInput2
            tensor1 = self.MANet(tensorPred ,ref_img, tensorH_next)
            tensorPred = tensorPred + tensor1
            return tensorPred

        if tensorLong is None:
            tensorLong = (torch.zeros(batch_size, 4, tensorInput1.size(2), tensorInput1.size(3))).cuda()
        tensorLong = self.moduleLong(tensorLong)
    ...
    # ------------------- Doceder Part -----------------
        ...
        tensorCombine = torch.cat([tensorCombine1, tensorLong], 1)
    ...
    # Return the predictd tensor and the next state of convLSTM
        ...
        if do_MA:
            if ref_img is None:
                ref_img = torch.zeros(tensorInput1.shape).cuda()
            tensor1 = self.MANet(tensorPred ,ref_img, tensorH_next)
            tensorPred = tensorPred + tensor1
        return tensorPred


    def load_my_state_dict(self, state_dict, flag=True):
