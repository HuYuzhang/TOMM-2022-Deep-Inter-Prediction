"""
I find that the warped result can bring in little improvement, so I think this pixel-level information should be fead from the head?
Try it in this version
"""
...
class MANet(nn.Module):
    def __init__(self, in_channels=6, out_channels=3, n_feat=32, kernel_size=3, stride=2, n_RRG=1, n_MSRB=2, height=3, width=2, bias=False):
        super(MANet, self).__init__()

        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)
        self.conv_combine = nn.Conv2d(n_feat*2, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)
    ...
    def forward(self, x, w, j):
        img_feat = self.conv_in(torch.cat([x,w], axis=1))
        combine_feat = torch.cat([img_feat, j], axis=1)
        combine_feat = self.conv_combine(combine_feat)

        out_feat = self.body(combine_feat)

        img_out = x + self.conv_out(out_feat)
